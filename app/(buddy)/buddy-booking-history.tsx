...
export default function BuddyBookingHistoryScreen() {
  const { colors } = useTheme();
  const { buddyBookings, isBuddyLoading, getBuddyBookingHistory } = useAppStore();
  ...
  const loadBookings = async () => {
    ...
    await getBuddyBookingHistory(filters);
  };
  ...
  const getFilteredBookings = () => {
    if (activeTab === 'all') return buddyBookings;
    return buddyBookings.filter((b: any) => b.status === activeTab);
  };
              ...
              {booking.rating?.score && (
                <View style={{
                  ...
                  <Text style={{ marginLeft: 6, fontSize: 13, color: colors.text, fontWeight: '600' }}>
                    User rated {booking.rating.score}/5
                  </Text>
